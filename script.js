...
const seoPages = {
  ...
  calculator: { title: 'Calculator preț | Pirat Industrial', description: 'Calculează o estimare orientativă pentru prelate și acoperiri în funcție de suprafață.' },
};
...
const sharedNav = () => `
  ...
  <nav class="main-nav" id="main-nav" aria-label="Navigare principală"><details class="nav-dropdown"><summary>Produse</summary><div class="dropdown-menu"><a href="prelate-industriale.html">Prelate industriale</a><a href="prelate-auto.html">Prelate auto</a><a href="copertine.html">Copertine</a><a href="acoperiri-spatii-comerciale.html">Acoperiri comerciale</a><a href="confectii-metalice.html">Confecții metalice</a><a href="arhitectura-textila.html">Arhitectură textilă</a></div></details><details class="nav-dropdown"><summary>Servicii</summary><div class="dropdown-menu"><a href="consultanta.html">Consultanță</a><a href="montaj.html">Montaj</a><a href="intretinere.html">Întreținere</a><a href="reparatii.html">Reparații</a><a href="inscriptionari.html">Inscripționări</a></div></details><details class="nav-dropdown"><summary>Media</summary><div class="dropdown-menu"><a href="galerie-foto.html">Galerie foto</a><a href="galerie-video.html">Galerie video</a></div></details><a href="calculator.html">Calculator preț</a><a href="contact.html">Contact</a><a class="nav-phone" href="tel:+40722750179">0722 750 179</a><a class="button button-small" href="contact.html">Cere ofertă <span>↗</span></a></nav></header><a class="calculator-float" href="calculator.html" aria-label="Deschide calculatorul de preț">Calculator preț <span>↗</span></a>`;
...
function renderInnerPage(pageId) {
  ...
  if (productPages[pageId]) {
    ...
    const options = Object.entries(productPages).map(([slug, page]) => `<option value="${slug}">${page.kicker.split(' / ')[1]}</option>`).join('');
    main = `<main id="top"><section class="inner-hero"><div class="layout"><p class="eyebrow">Calculator</p><h1>Calculează o estimare orientativă.</h1><p>Introdu dimensiunile și află un preț estimativ pe baza tarifelor publicate.</p></div></section><section class="calculator layout"><form class="calculator-form" id="price-calculator"><label>Tipul lucrării<select name="product" required>${options}</select></label><label>Lățime, metri<input name="width" type="number" min="0.1" step="0.01" required placeholder="Ex. 5" /></label><label>Lungime, metri<input name="length" type="number" min="0.1" step="0.01" required placeholder="Ex. 8" /></label><label>Cantitate<input name="quantity" type="number" min="1" step="1" value="1" required /></label><button class="button" type="submit">Calculează estimarea <span>↗</span></button></form><div class="calculator-result" id="calculator-result" aria-live="polite"><p class="eyebrow">Estimare</p><h2>Completează dimensiunile.</h2><p>Rezultatul este orientativ și nu înlocuiește oferta personalizată.</p></div></section>${contactBlock()}</main>`;
...
function wireInteractions() {
  ...
  document.querySelector('#price-calculator')?.addEventListener('submit', (event) => {
    event.preventDefault();
    const form = event.currentTarget;
    const data = new FormData(form);
    ...
    if (!rate) {
      result.innerHTML = '<p class="eyebrow">Estimare</p><h2>Tarif indisponibil.</h2><p>Trimite-ne dimensiunile pentru o ofertă personalizată.</p>';
      return;
    }
    const total = area * rate;
    result.innerHTML = `<p class="eyebrow">Estimare orientativă</p><h2>${new Intl.NumberFormat('ro-RO', { style: 'currency', currency: 'RON' }).format(total)}</h2><p>${area.toFixed(2)} m² × ${rate.toFixed(2)} lei/m²</p>`;
  });
}
